...
USAGE="Usage: ./benchmark.sh [iter=[num]] [KITTI Dataset Sequence Numbers]\n"
...
BMCPLUSPLUSCMD00_02="./mono_kitti ../../Vocabulary/ORBvoc.txt ./KITTI00-02.yaml ../../data/data_odometry_color/dataset/sequences/"
BMCPLUSPLUSCMD03="./mono_kitti ../../Vocabulary/ORBvoc.txt ./KITTI03.yaml ../../data/data_odometry_color/dataset/sequences/"
BMCPLUSPLUSCMD04_12="./mono_kitti ../../Vocabulary/ORBvoc.txt ./KITTI04-12.yaml ../../data/data_odometry_color/dataset/sequences/"
if [ "$BMCPLUSPLUS" = true ] ; then
  ...
  # Extract run time from log
  echo -e "\nExtracting Run Times from logs..."
  if [ -f "$RESULTCPLUSPLUSDIR/runtimes.txt" ] ; then
    rm -f "$RESULTCPLUSPLUSDIR/runtimes.txt"
  fi
  echo -ne " \t\tRun Times\nSeq #\tRun Time\n" \
    > "$RESULTCPLUSPLUSDIR/runtimes.txt"
  for seqlog in $(ls -al "$RESULTCPLUSPLUSDIR/logs" | egrep -oh "(0[0-9]|10).log$") ; do
    echo -ne "$seqlog" | egrep -oh "^(0[0-9]|10)" | tr -d '\n'\
      >> "$RESULTCPLUSPLUSDIR/runtimes.txt"
    ...
    egrep -h "^Time taken by function" "$RESULTCPLUSPLUSDIR/logs/$seqlog" | \
      egrep -oh "[0-9].[0-9]+$" | tr -d '\n' \
      >> "$RESULTCPLUSPLUSDIR/runtimes.txt"
