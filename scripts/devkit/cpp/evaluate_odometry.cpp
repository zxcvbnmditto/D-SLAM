...
bool eval (const string &result_dir) {
  ...
  string gt_dir         = result_dir + "/poses";
  string data_dir     = result_dir + "/trajectories";
  ...
  for (int32_t i=0; i<11; i++) {
    ...
    vector<Matrix> poses_result = loadPoses(data_dir + "/" + file_name);
   
    // plot status
    // mail->msg("Processing: %s, poses: %d/%d",file_name,poses_result.size(),poses_gt.size());
    cout << "Processing " << file_name << " that contains "
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=2) {
    cout << "Usage: ./eval_odometry result_directory" << endl;
    return 1;
  }
